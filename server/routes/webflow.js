  ...
  try {
    ...
    if (userToken) {
      ...
      if (response.ok) {
        ...
        res.render('webflow', overrideParams(defaultParams, { user: data.username }));
      } else {
        res.render('webflow', overrideParams(defaultParams, { user: "error" }));
      }
    } else {
      res.render('webflow', defaultParams);
    }
  } catch (error) {
